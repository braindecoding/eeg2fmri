...
# Custom collate function untuk mixed dataset shapes
def custom_collate_fn(batch):
    """Custom collate function untuk handle different EEG shapes dalam satu batch"""
    
    # Separate by dataset type
    mindbig_samples = [item for item in batch if item['dataset_type'] == 'mindbigdata']
    crell_samples = [item for item in batch if item['dataset_type'] == 'crell']
    
    # Create separate batches
    batched_data = {}
    
    if mindbig_samples:
        # Stack MindBigData samples
        mindbig_eeg = torch.stack([item['eeg_data'] for item in mindbig_samples])
        mindbig_fmri = torch.stack([item['synthetic_fmri_target'] for item in mindbig_samples])
        mindbig_stimulus = torch.stack([item['stimulus_image'] for item in mindbig_samples])
        mindbig_codes = [item['stimulus_code'] for item in mindbig_samples]
        mindbig_types = [item['dataset_type'] for item in mindbig_samples]
        
        batched_data['mindbigdata'] = {
            'eeg_data': mindbig_eeg,
            'synthetic_fmri_target': mindbig_fmri,
            'stimulus_image': mindbig_stimulus,
            'stimulus_code': mindbig_codes,
            'dataset_type': mindbig_types
        }
    
    if crell_samples:
        # Stack Crell samples
        crell_eeg = torch.stack([item['eeg_data'] for item in crell_samples])
        crell_fmri = torch.stack([item['synthetic_fmri_target'] for item in crell_samples])
        crell_stimulus = torch.stack([item['stimulus_image'] for item in crell_samples])
        crell_codes = [item['stimulus_code'] for item in crell_samples]
        crell_types = [item['dataset_type'] for item in crell_samples]
        
        batched_data['crell'] = {
            'eeg_data': crell_eeg,
            'synthetic_fmri_target': crell_fmri,
            'stimulus_image': crell_stimulus,
            'stimulus_code': crell_codes,
            'dataset_type': crell_types
        }
...
def create_data_loaders(datasets_dir: str, batch_size: int = 8) -> Tuple[DataLoader, DataLoader]:
    ...
    print(f"✓ Created data loaders with custom collate function:")
...
def train_ntvit_model(datasets_dir: str, 
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            # Handle new batch structure from custom collate function
            
            # Train MindBigData model
            if 'mindbigdata' in batch:
                mindbig_data = batch['mindbigdata']
                mindbig_eeg = mindbig_data['eeg_data'].to(device)
                mindbig_target = mindbig_data['synthetic_fmri_target'].to(device)
                ...
                total_loss.backward()
                torch.nn.utils.clip_grad_norm_(mindbig_model.parameters(), 1.0)
            ...
            # Train Crell model
            if 'crell' in batch:
                crell_data = batch['crell']
                crell_eeg = crell_data['eeg_data'].to(device)
                crell_target = crell_data['synthetic_fmri_target'].to(device)
                ...
                total_loss.backward()
                torch.nn.utils.clip_grad_norm_(crell_model.parameters(), 1.0)
